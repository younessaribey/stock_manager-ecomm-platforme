  ...
  const fetchCategories = async () => {
    try {
      ...
      const organizedCategories = mainCategories.map(mainCat => ({
        ...mainCat,
        subcategories: response.data.filter(cat => cat.parentId === mainCat.id)
      }));
      ...
      <div className="bg-white rounded-lg shadow-sm">
        <div className="p-6 border-b border-gray-200">
          ...
          <p className="text-sm text-gray-600 mt-1">
            {categories.length} main categories • {categories.reduce((acc, cat) => acc + (cat.subcategories?.length || 0), 0)} subcategories
          </p>
        </div>
